...
def start_server():
    ...
    try:
        # Start the HTTP server in the specified directory (completely hidden)
        startupinfo = subprocess.STARTUPINFO()
        startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
        startupinfo.wShowWindow = subprocess.SW_HIDE
        
        server_process = subprocess.Popen(
            ["python", "-m", "http.server", str(port_num)],
            cwd=directory,
...
try:
    root.iconbitmap("D:\Project_FD\ProgramingPROj\web3\project_rose_server\icon.ico")
except:
